...
module.exports = class {
    ...
    async run (message) {
        ...
        if(this.client.config.proMode && message.guild){
            if((!this.client.config.proUsers.includes(message.guild.ownerID) || this.client.guilds.filter((g) => g.ownerID === message.guild.ownerID) > 1) && message.guild.ownerID !== this.client.config.owner.id){
            }
        }
